...
import CaamFlightManagementSampleDDatatable from "./CaamFlightManagementSampleDDataTable";
...
import CaamFlightManagementSampleDFakerDialogComponent from "./CaamFlightManagementSampleDFakerDialogComponent";
import CaamFlightManagementSampleDSeederDialogComponent from "./CaamFlightManagementSampleDSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const CaamFlightManagementSampleDPage = (props) => {
    ...
    const filename = "caamFlightManagementSampleD";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("caamFlightManagementSampleD");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("caamFlightManagementSampleD")
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "CAAM Flight Management Sample D", type: "error", message: error.message || "Failed get CAAM Flight Management Sample D" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("caamFlightManagementSampleD").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "CAAM Flight Management Sample D", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("caamFlightManagementSampleD").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "CAAM Flight Management Sample D",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/caamFlightManagementSampleD/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>CAAM Flight Management Sample D </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="caamFlightManagementSampleD-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="caamFlightManagementSampleD-datatable">
                    <CaamFlightManagementSampleDDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CaamFlightManagementSampleDEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CaamFlightManagementSampleDCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CaamFlightManagementSampleDFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CaamFlightManagementSampleDSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CaamFlightManagementSampleDPage);
