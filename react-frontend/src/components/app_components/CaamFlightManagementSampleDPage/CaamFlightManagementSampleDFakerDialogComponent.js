...
import caamFlightManagementSampleDFakerFactory from "./caamFlightManagementSampleDFakerFactory";
...
const CaamFlightManagementSampleDFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = caamFlightManagementSampleDFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("caamFlightManagementSampleD").create(elem));
...
export default connect(mapState, mapDispatch)(CaamFlightManagementSampleDFakerDialogComponent);
